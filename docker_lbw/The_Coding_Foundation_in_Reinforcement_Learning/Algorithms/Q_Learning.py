...
# class OnPolictyQL:
#     def __init__(self, env, start_state, alpha=0.1, gamma=0.9, epsilon=0.1, iterations=100):
#         self.env = env
...
#         self.policy = self.policy / self.policy.sum(axis=1)[:, np.newaxis]
...
#                     self.q[s, a] - (reward + self.gamma * np.max(self.q[next_state])))  # solving the Bellman optimality equation
#             s = next_state
...
def on_policy_ql(env, start_state = (0, 0), alpha=0.1, gamma=0.9, epsilon=0.1, iterations=100):
